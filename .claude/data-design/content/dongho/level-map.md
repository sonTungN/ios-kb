...
**One economy, three sequential chapters.** The three eras are playable **levels** over a single
unbroken economy: a chapter unlocks when the previous one ends, opens on its era screen, closes on
a summary screen, and **nothing resets between them** — stats, flags, ticks and spent crises carry
over, so every proof in this file covers the chaptered game unchanged. `BRIEF-05` §3 is satisfied
on the direct "levels" reading; the Chọn di sản screen's "Gốm · Quan họ — sắp ra mắt" tiles extend
the dimension. Chapter objectives: C1 `giu_van`+`giu_bi_quyet` · C2 ticks 2–3 of `truyen_thua` ·
C3 `phuc_hoi_van`+`truyen_nhan`+`ho_so`.
...
| 21 | 1989ᵃ | `lich-tran-ve` | Offset calendars flood Tết `S` | pressure | hạ giá đấu lịch `0/−5/+5/0` | giữ giá mặc kệ `+5/−5/−10/0` | omen before |
...
3. **Chapter III band pressure is deliberately hot** (fame-era ceilings) — the canonical Thường line
   brushes a ceiling once (offer accepted — nghe 80→60, tieng −5). Playtest the ceiling texts.
...
36 spine + 7 weave + 2 banners + ≤3 ambient + ~9 interstitials + 0–8 crises ≈ **50–62 screens ·
...
## What the app already reads (state on 08/09/2026)

The engine is built and plays this map end to end. Three files ship in the bundle, and the copies
in this workspace are byte identical to them: `game.json`, `game_contents.json`, and
`chapter_prescreens.json` (the chapter briefing, three pages per chapter, authored after this map
was written and now the screen every chapter door opens on).

What the engine reads today, and what it substitutes:

| Content | State | Consequence in the app |
| --- | --- | --- |
| Weave 17, ambient 8, crises 8, interstitials 14, trial | authored, with `effectReasons` on every nonzero effect | plays with real prose and real why-chips |
| 36 spine cards (`game_spine.json`, ids `NN-<id>`) | **written 08/09/2026**, linter 0 errors, sim WINS on Thường and Khó; `verifiedBy` still empty on every source | plays with real prose and real why-chips; the DEBUG placeholder is never used once the files are bundled |
| Card illustrations | complete | 36 spine, 17 weave, 8 ambient, 8 crisis, 9 briefing pages, named `NN-<card-id>` |

A `[Nháp]` or `[Draft]` line can now only mean a card file failed to decode, never a missing
mechanism. Card 7 carries `showYear: false` so its ordering year is never shown on the card, its outcome or a ritual caption; its `speaker` is the voice of the prompt and nothing more (09/09/2026: every spine card also carries a bilingual `title`, shown under the year).

**Card 21 id.** The map, the sim and the placeholder now all say `lich-tran-ve`, matching the
shipped asset `21-lich-tran-ve`. The earlier id `lich-offset-tran-ve` never resolved to a picture.

**Chapter names live in `chapter_prescreens.json`.** Settled 08/09/2026: that file is the only
one carrying all three chapters in both languages, so it owns the names. `ChapterInfo.swift`
decodes it once and reads `title` and `period` from it (falling back to this map only if the file
cannot be read), and the chapter II banner in `game_contents.json` was changed from "Keep the Way"
to "Keeping the Tradition" to match. One set of names now, everywhere:

| Chapter | vi | en |
| --- | --- | --- |
| I | Giữ lửa | Keeping the Flame |
| II | Giữ nếp | Keeping the Tradition |
| III | Hồi sinh | Revival |

Rename a chapter by editing that JSON alone. Nothing in Swift needs to change.
...
- **Spine prose**: 36 cards × 6 blocks × 2 languages = **432 strings** (each card cites `research/01`),
  plus an `effectReasons` chip (vi+en, ≤ 8 từ) for every nonzero effect — ≈ **250–300 micro-strings**
  written together with the cards (`design/game_strategy_and_logic.md` §4). Weave and crisis chips
  are already in `game_contents.json`.
...
- **Validator follow-ups**: a `minigame` content check (that every `contentRef` names a real
  painting in the codex). The draw, economy and reasons laws are live — including the linter's
  deck-activated copies of the faucet/drain law and the `chose:` tag resolution, which begin
  firing now that `game_spine.json` exists. See `design/game_strategy_and_logic.md` §5 for the full
  guarantee-to-enforcement table. The card-33 conditional pair is now modelled — `ho_so` declares
